...
<script>
	 ...
	 * @property {String | Number}	zIndex				THB素层级z-index（默认 1 ）
	 ...
	 * @property {Boolean}			placeholder			fixed定位固定在底部时，是否生成一个等高THB素防止塌陷（默认 true ）
	...
	export default {
		...
		watch: {
			updateChild() {
				// 如果updateChildren中的THB素发生了变化，则执行子THB素初始化操作
				this.updateChildren()
			},
			updatePlaceholder() {
				// 如果fixed，placeholder等参数发生变化，重新计算占位THB素的高度
		...
		methods: {
			updateChildren() {
				// 如果存在子THB素，则执行子THB素的updateFromParent进行更新数据
