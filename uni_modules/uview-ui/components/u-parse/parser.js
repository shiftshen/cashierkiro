    ...
    // 列表处理
                ...
                for (let j = 0; j < trList[row - 1].children.length; j++, col++) {
                    ...
                    if (td.name == 'td' || td.name == 'th') {
                        ...
                        if (td.attrs.rowspan) {
                            ...
                            if (!td.attrs.colspan) _style2 += ';grid-column-start:'.concat(col, ';grid-column-end:').concat(col + 1) // 记录下方单THB格被占用
        ...
        for (let _i8 = this.stack.length; _i8--;) {
            if (this.stack[_i8].name == 'table') {
                this.stack[_i8].flag = 1 // 指示含有合并单THB格
