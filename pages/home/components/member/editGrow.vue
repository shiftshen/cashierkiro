		...
		<u-overlay :show="show">
			<view class="mode f20 bf" style="width:610px">
				<view class="p15 bd1 dfbc">
					<text>{{$t('member.adjust_growth_value')}}</text><text class="iconfont icon-cuowu" @click="show=false"></text>
				</view>
				<view class="p15 f-y-bt">
					<view class="f-1 f-c-ac">
						<u--form class="mb10" ref="uForm" :model="bForm" :rules="rules" :labelWidth="120"
							:labelStyle="{fontSize:'18px'}">
							<u-form-item :label="$t('member.current_growth_value_seedel')" prop="integral" ref="item1">
							...
							<u-form-item :label="$t('member.modification_content')" prop="value" ref="item1">
								<view class="flex f-y-c">
									<view class="flex mr20 f15">{{ bForm.type == 1 ? $t('member.growth_value_reissue') :$t('member.growth_value_deduction')}}</view>
								...
								<view class="f-s-bt f18">
									...
									<view class="" style="width:230px">
										<u--textarea v-model="bForm.notes" height="100"
											:placeholder="$t('member.please_enter_remarks_seedel')"></u--textarea>
									</view>
								</view>
							</u-form-item>
						</u--form>
					</view>
					<u-button color="#4275F4" :text="$t('member.confirm')" @click="show=false"></u-button>
				</view>
			</view>
		</u-overlay>
	</view>
