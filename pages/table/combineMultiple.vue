	...
	<view class="f-y-bt w100v h100v o-h">
		...
		<view class="right f-1 p10 l-h1" style="background: #eff0f4;">
			...
			<view class="tables">
				<view class="table mr15 mb15 bf pt15 f-y-bt"
					...
					<view class="p10 f-x-bt f14 bottom" style="background: rgba(#000,.3)">
						...
						<view class="mode f-c-c" v-if="ids.includes(v.id)">
							<view class="cf f18">
								 {{$t('table.merge')}}
							</view>
			...
			<view class="p-5-10 f-x-bt bs6 bf kinds f20">
				<view class="">
					{{$t('table.select_table_to_merge')}} 
				</view>
				<view class="dfac">
					<u-button class="mr15" :text="$t('modal.cancelText')" :customStyle="{width:'100px'}"
						@click="cancle"></u-button>
					<u-button color="#4275F4" :disabled="ids.length==0" :customStyle="{width:'100px'}" @click="confirm"><text
...
<script>
	...
	export default {
		...
		methods: {
			...
			clickItem(v, i) {
				const maxSelect = 20; 
				const index = this.ids.indexOf(v.id);
				if (index === -1) {
					if (this.ids.length < maxSelect) {
						this.ids.push(v.id);
					} else {
						uni.showToast({
							title: `${this.$t('table.maximum_selection_allowed')}${maxSelect}table`,
							icon: 'none'
						});
					}
				} else {
			...
			async confirm() {
				if (this.ids.length == 0) {
					uni.showToast({
						title: this.$t('table.select_table_to_merge'),
						icon: 'none'
					})
					return;
				}
